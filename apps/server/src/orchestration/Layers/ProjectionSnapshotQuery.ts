...
import { ProjectionThreadProviderItemRepositoryLive } from "../../persistence/Layers/ProjectionThreadProviderItems.ts";
...
function toProjectedThreadRuntime(row: ProjectionThreadRuntime): OrchestrationThreadRuntime {
  return {
    threadId: row.threadId,
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const projectionThreadProviderItemRepository = yield* ProjectionThreadProviderItemRepository;
      ...
      .withTransaction(
        Effect.gen(function* () {
          ...
          const runtimeByThread = new Map(
            runtimeRows.map((row) => [row.threadId, toProjectedThreadRuntime(row)] as const),
          );
          ...
          return Option.some(
            toProjectedThreadShellFromStoredSummary({
              ...
              runtime: Option.match(runtimeRow, {
                onNone: () => null,
                onSome: (row) => toProjectedThreadRuntime(row),
              }),
            }),
          );
        }),
  ...
  const loadThreadDetail = (
    ...
    Effect.gen(function* () {
      ...
      const [
        ...
        projectionThreadProviderItemRepository.listByThreadId({ threadId }),
      ...
      const thread = toProjectedThread({
        ...
        providerItems: providerItemRows.map((row) => row.item),
        ...
        runtime: Option.match(runtimeRow, {
          onNone: () => null,
          onSome: toProjectedThreadRuntime,
        }),
      });
...
export const OrchestrationProjectionSnapshotQueryLive = Layer.effect(
  ...
  Layer.provideMerge(ProjectionThreadProviderItemRepositoryLive),
