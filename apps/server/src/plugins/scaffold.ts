...
function pluginName(target: string): string {
  const name = path.basename(path.resolve(target)).toLowerCase().replace(/[^a-z0-9-]+/g, "-");
...
export function scaffoldPlugin(target: string): { readonly id: string; readonly sourceRoot: string } {
    ...
    `${JSON.stringify(
      {
        ...
        synara: {
          displayName: name.replace(/(^|-)([a-z])/g, (_, prefix: string, letter: string) => `${prefix ? " " : ""}${letter.toUpperCase()}`),
          apiVersion: 1,
