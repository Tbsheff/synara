...
function modelSelectionsMatch(left: ModelSelection | undefined, right: ModelSelection | undefined) {
        ...
        return yield* lifecycle.run(threadId, (lease) =>
          Effect.gen(function* () {
            ...
            const persistedModelSelection = persistedBinding
              ...
              : undefined;
            const canReusePersistedSession =
              persistedBinding?.provider === input.provider &&
              (input.modelSelection === undefined ||
                modelSelectionsMatch(input.modelSelection, persistedModelSelection));
            const effectiveResumeCursor =
              input.resumeCursor ??
              (canReusePersistedSession ? persistedBinding?.resumeCursor : undefined);
