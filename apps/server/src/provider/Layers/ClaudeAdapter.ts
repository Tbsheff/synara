...
function describeClaudeTaskUpdate(message: ClaudeTaskUpdatedMessage): string {
  ...
  return message.patch.status ? `Task ${message.patch.status}` : "Task updated";
}
...
function completedClaudeTaskStatus(
  ...
  return status === "killed" ? "stopped" : undefined;
}
  ...
  return Effect.gen(function* () {
    ...
    const ensureReasoningBlock = (
      ...
      Effect.gen(function* () {
        ...
        const block: ReasoningBlockState = { itemId, blockIndex, completionEmitted: false };
        ...
        yield* offerRuntimeEvent(context, {
          ...
          payload: {
            ...
            status: "inProgress",
            title: "Thinking",
            data: { contentIndex: blockIndex },
          },
          providerRefs: nativeProviderRefs(context, { providerItemId: itemId }),
          raw: { source: "claude.sdk.message", method: raw.method, payload: raw.payload },
        });
        return block;
      });
      ...
      Effect.gen(function* () {
        ...
        yield* offerRuntimeEvent(context, {
          ...
          payload: {
            ...
            status,
            title: "Thinking",
            data: { contentIndex: block.blockIndex },
          },
    ...
    const handleSystemMessage = (
      ...
      Effect.gen(function* () {
        ...
        if (message.subtype === "task_updated") {
          ...
          if (!context.knownWorkflowTaskIds.has(message.task_id)) {
            const completedStatus = completedClaudeTaskStatus(status);
            const summary = patch.error?.trim() || (status ? `Status: ${status}` : undefined);
            const lifecycleStamp = yield* makeEventStamp();
            yield* offerRuntimeEvent(context, {
              type: completedStatus ? "task.completed" : "task.progress",
